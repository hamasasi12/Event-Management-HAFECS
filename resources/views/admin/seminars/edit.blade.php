<x-layouts.admin title="Edit Seminar">
    <!-- Breadcrumb -->
    <livewire:breadcrumb :page-title="'Edit Seminar '" :breadcrumbs="[
    ...
    <div class="bg-white shadow overflow-hidden rounded-lg dark:bg-gray-800">
        <div class="px-4 py-5 sm:p-6">
            <form action="{{ route('admin.seminars.update', ['seminar_hashid' => $seminar->hashid]) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <input type="text" name="title" id="title" value="{{ old('title', $seminar->title) }}" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                    </div>
                    ...
                    <div>
                        ...
                        <input type="datetime-local" name="start_time" id="start_time" value="{{ old('start_time', $seminar->start_time->format('Y-m-d\TH:i')) }}" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                    </div>

                    <!-- End Time -->
                    <div>
                        <label for="end_time" class="block text-sm font-medium text-gray-700 dark:text-gray-300">End Time</label>
                        <input type="datetime-local" name="end_time" id="end_time" value="{{ old('end_time', $seminar->end_time->format('Y-m-d\TH:i')) }}" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                    </div>
                    ...
                    <div>
                        ...
                        <input type="number" name="price" id="price" value="{{ old('price', $seminar->price) }}" step="0.01" min="0" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                    </div>
                    ...
                    <div>
                        ...
                        <select name="platform" id="platform" class="mt-1 block w-full bg-white border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                            ...
                            <option value="Zoom" {{ old('platform', $seminar->platform) == 'Zoom' ? 'selected' : '' }}>Zoom</option>
                            <option value="Google Meet" {{ old('platform', $seminar->platform) == 'Google Meet' ? 'selected' : '' }}>Google Meet</option>
                            <option value="Microsoft Teams" {{ old('platform', $seminar->platform) == 'Microsoft Teams' ? 'selected' : '' }}>Microsoft Teams</option>
                            <option value="Webex" {{ old('platform', $seminar->platform) == 'Webex' ? 'selected' : '' }}>Webex</option>
                            <option value="YouTube Live" {{ old('platform', $seminar->platform) == 'YouTube Live' ? 'selected' : '' }}>YouTube Live</option>
                            <option value="Other" {{ old('platform', $seminar->platform) == 'Other' ? 'selected' : '' }}>Other</option>
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="status" id="status" class="mt-1 block w-full bg-white border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                            <option value="upcoming" {{ (old('status', $seminar->status) == 'upcoming') ? 'selected' : '' }} class="dark:bg-gray-700 dark:text-white">Upcoming</option>
                            <option value="active" {{ (old('status', $seminar->status) == 'active') ? 'selected' : '' }} class="dark:bg-gray-700 dark:text-white">Active</option>

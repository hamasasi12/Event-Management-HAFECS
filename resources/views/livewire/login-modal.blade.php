    ...
    <div x-data="{ show: @entangle('isOpen') }" x-show="show" x-transition:enter="transition ease-out duration-300"
        ...
        <div x-show="show" x-transition:enter="transition ease-out duration-300"
            ...
            <div class="px-8 py-10 relative z-10 bg-white">
                ...
                <form action="{{ route('login') }}" method="POST" class="mt-6 space-y-4 relative z-10">
                    @csrf
                    <div>
                        <label for="email" class="sr-only">Email address</label>
                        <input id="email" name="email" type="email" required
                            class="appearance-none relative block w-full px-4 py-3 border border-gray-300 placeholder-gray-400 text-gray-900 rounded-xl focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm bg-gray-50"
                    ...
                    <div x-data="{ showPassword: false }" class="relative">
                        ...
                        <button type="button" @click="showPassword = !showPassword"
                            ...
                            <svg x-show="showPassword" style="display: none;" class="h-5 w-5" fill="none"
                ...
                <div class="mt-8 text-center text-sm">
                    <p class="text-gray-600">Belum punya akun? <button
                            @click="$dispatch('open-register-modal'); show = false;"
                            class="font-bold text-blue-500 hover:text-blue-400 transition-colors focus:outline-none">Sign
                            up</button></p>
                </div>
            </div>
        </div>

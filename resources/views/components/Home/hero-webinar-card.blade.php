...
@php
    ...
    $recentRegistrants = $seminar
        ? $seminar->registrations()->with('user')->latest()->take(4)->get()
        : collect();
            ...
            <div class="flex flex-col sm:flex-row items-center justify-between bg-blue-50 p-4 rounded-2xl gap-4 sm:gap-0">
                    ...
                    <div class="flex items-center -space-x-3">
                        @forelse ($recentRegistrants as $index => $reg)
                            ...
                            @if ($userAvatar)
                                ...
                                <img
                                    src="{{ Str::startsWith($userAvatar, 'http') ? $userAvatar : asset('storage/' . $userAvatar) }}"
                                    alt="{{ $name }}"
                                    title="{{ $name }}"
                        ...
                        @empty
                            ...
                            <div class="w-8 h-8 sm:w-9 sm:h-9 rounded-full border-2 border-white bg-slate-300 flex items-center justify-center text-[10px] font-bold text-white shadow-sm">
                                ?
                            </div>
                        @endforelse
                ...
                <div class="text-right w-full sm:w-auto flex justify-center sm:block">
                    <div class="inline-flex items-center justify-center px-4 py-2 bg-white rounded-xl shadow-sm border border-slate-100">
                        <span class="text-lg font-black text-blue-600">{{ $seminar ? number_format($totalCount, 0, ',', '.') : '0' }}</span>
